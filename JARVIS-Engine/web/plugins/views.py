...
from web.jarvis.settings import JARVIS_PATH
        ...
        for item in json.loads(metareq.text or metareq.content):
            for plugin in Plugin.objects(name=item['name']):
                ...
                if plugin.version < item['version']:
                    item['upgrade'] = True
            if os.path.exists(JARVIS_PATH + '/Plugins/' + item['name']):
                item['installed'] = True
            plugins.append(item)
...
def uninstall(request):
    ...
    for plugin in Plugin.objects(name=plugin_name):
        plugin.delete()
    rmtree(JARVIS_PATH + '/Plugins/' + plugin_name)
    return HttpResponse('OK')
...
def install(request):
    ...
    metadata = json.load(open(JARVIS_PATH + '/Plugins/' + plugin_name + '/' + str(plugin_name).lower() + '.json'))
    plugin = Plugin()
