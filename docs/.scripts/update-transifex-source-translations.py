...
def main():
    ...
    handled_resources = set(
        ...
        cfg.sections() if section.startswith('lisa.docs')
    )
    ...
    for idx, (resource_path, resource_name) in enumerate(sorted(entries)):
        ...
        try:
            txclib.utils.exec_command(
                'set',
                '--auto-local -r lisa.docs-{resource_name} '
            ...
            sys.stdout.write('\n')
            if 'lisa.docs-{0}'.format(resource_name) in handled_resources:
                handled_resources.remove('lisa.docs-{0}'.format(resource_name))
    ...
    if 'TRANSIFEX_NO_PUSH' not in os.environ:
        ...
        for idx, (resource_path, resource_name) in enumerate(sorted(entries)):
            ...
            try:
                txclib.utils.exec_command(
                    'push',
                    '--resource lisa.docs-{resource_name} '
                    '--source '
                    '--skip '
